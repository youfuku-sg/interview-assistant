...
- [ ] 2.3 AI レスポンスを改行分割し、先頭の記号・番号を除去して質問配列を得るパーサーを実装する。分割結果がちょうど5件でなければ生レスポンスを1件として返すフォールバックを実装する
- [ ] 2.4 AI プロバイダー未設定時（`selectedAIProvider.provider` 未設定 かつ Pluely API 無効相当）はリクエストを送信せず、案内表示用のフラグ/メッセージを返す
- [ ] 2.5 `fetchAIResponse` が例外・空レスポンス・既存のエラーテキスト出力で終了した場合を生成失敗として扱い、質問配列を返さない
