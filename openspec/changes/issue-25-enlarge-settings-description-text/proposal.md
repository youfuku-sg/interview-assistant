...
- `src/pages/settings/components/DeleteChats.tsx`: `Header` に `descriptionClassName` を渡す(このコンポーネントには個別の `<p>` 説明文はないため対象外。削除確認ダイアログ内の `text-sm text-muted-foreground` はモーダル内の確認文言であり、Issue が指す「設定画面の各項目の説明文」の対象外として変更しない)。
