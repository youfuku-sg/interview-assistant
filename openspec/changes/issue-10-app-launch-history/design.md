...
- **Record the launch inside `run()`'s `.setup()` closure in `src-tauri/src/lib.rs`**, once per process start. This fires exactly once per app launch regardless of how many windows (overlay/dashboard) are created afterward — recording on window creation would risk double-counting since `setup` already pre-creates the dashboard window.
  ...
  - **Concrete approach:** open a separate `sqlx::SqlitePool` (already a transitive dependency at `sqlx 0.8.6` per `Cargo.lock`, so this is not a new library, just a new direct `Cargo.toml` entry and a new *connection*) pointed at the same file `tauri-plugin-sql` migrates. `sqlite:pluely.db` is resolved by the plugin relative to the app's data directory (the same `app_data_dir()` used elsewhere in this codebase, e.g. `activate.rs`/`shortcuts.rs`) — implementation must verify and reuse that exact resolved path so the two connections address the same file, and must run after migrations have applied (the plugin runs migrations during plugin registration, which happens before `.setup()` executes, so ordering should be safe, but this should be confirmed rather than assumed).
