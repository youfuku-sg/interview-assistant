...
- Expose the 10 most recent entries to the frontend via a parameterless Tauri command.
...
- **Record the launch inside `run()`'s `.setup()` closure in `src-tauri/src/lib.rs`**, once per process start. This fires exactly once per app launch regardless of how many windows (overlay/dashboard) are created afterward — recording on window creation would risk double-counting since `setup` already pre-creates the dashboard window.
  ...
  - **Concrete approach:** inside `.setup()`, use `tauri::async_runtime::block_on` for the short database operation (the map is behind a `tokio::sync::RwLock`, so accessing it requires an async context); within that block, fetch `app.state::<tauri_plugin_sql::DbInstances>()`, `.0.read().await`, look up key `"sqlite:pluely.db"`, and match `DbPool::Sqlite(pool) => ..` to obtain the live `&sqlx::SqlitePool`; then run `sqlx::query("INSERT INTO app_launches DEFAULT VALUES").execute(pool).await`. Propagate lookup, pool-type, and SQL failures from `.setup()` instead of detaching a fire-and-forget task. This guarantees the current launch is committed before the frontend can invoke the history command and prevents an early process exit from dropping the record. `sqlx` still needs to move from a transitive to a **direct** `Cargo.toml` dependency (matching the already-resolved `0.8.6` line in `Cargo.lock`; only the `sqlite` feature is needed, not a runtime/connection feature) purely so app code can call `sqlx::query(..)` against the borrowed pool — it opens nothing itself.
- **New parameterless Tauri command `get_app_launch_history() -> Vec<AppLaunchEntry>`**, returning exactly the 10 most recent available entries via `ORDER BY launched_at DESC, id DESC LIMIT 10` so launches recorded in the same second have deterministic ordering. A fixed limit matches Issue #10 and avoids an unnecessary signed IPC parameter whose zero, negative (SQLite interprets a negative `LIMIT` as unbounded), or oversized values would otherwise need a separate validation contract. Added alongside existing simple commands (e.g. near `get_app_version` in `lib.rs`, or in a small new `src-tauri/src/app_launch.rs` module if `lib.rs` is judged too cluttered by the time of implementation) rather than piggy-backing on `api.rs`, since `api.rs` is reserved for chat/STT/provider concerns per the OpenWiki "where to change things" table. This command looks up the same borrowed `&sqlx::SqlitePool` from `app.state::<tauri_plugin_sql::DbInstances>()` on each invocation (via the `AppHandle`/`State` argument Tauri injects into commands) the same way as the startup insert, rather than holding or opening any pool of its own.
...
- Exact display format for timestamps in the Settings list (relative "◯分前" vs. absolute local datetime) — left to implementation to match existing date formatting conventions used elsewhere in the app (e.g. chat list timestamps), if any exist. `launched_at` is stored as Unix **seconds**, unlike the millisecond timestamps produced by `Date.now()` in existing chat code, so the frontend must multiply it by 1,000 before constructing a JavaScript `Date`.
