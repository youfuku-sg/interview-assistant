...
- **Record the launch inside `run()`'s `.setup()` closure in `src-tauri/src/lib.rs`**, once per process start. This fires exactly once per app launch regardless of how many windows (overlay/dashboard) are created afterward — recording on window creation would risk double-counting since `setup` already pre-creates the dashboard window.
  - **Important correction (found during review):** there is no existing precedent in this codebase for issuing SQL from Rust. Every current read/write to `pluely.db` goes through the frontend via `@tauri-apps/plugin-sql`'s `Database.load()` (`src/lib/database/config.ts`); grep of `src-tauri/src` shows `tauri_plugin_sql` referenced only in `lib.rs`/`db/main.rs` for migration registration, never for executing a query. `tauri-plugin-sql` v2 does not publicly expose its internal connection pool for reuse by application Rust code, so a Rust-side insert in `setup()` cannot simply "match how other startup work is done" — none of the other `setup()` steps touch the database at all.
  - **Concrete approach:** open a separate `sqlx::SqlitePool` (already a transitive dependency at `sqlx 0.8.6` per `Cargo.lock`, so this is not a new library, just a new direct `Cargo.toml` entry and a new *connection*) pointed at the same file `tauri-plugin-sql` migrates. For SQLite, the plugin resolves `sqlite:pluely.db` relative to Tauri's `BaseDirectory::AppConfig`, so app code must use `app.path().app_config_dir()?.join("pluely.db")` (not `app_data_dir()`) and construct `SqliteConnectOptions` from that path. The existing `plugins.sql.preload: ["sqlite:pluely.db"]` in `src-tauri/tauri.conf.json` causes the plugin to load the database and apply registered migrations during plugin initialization, before `.setup()` executes; retain that preload and verify this ordering while implementing.
- **New Tauri command `get_app_launch_history(limit: Option<i64>) -> Vec<AppLaunchEntry>`**, defaulting to 10 when `limit` is omitted, returning entries ordered newest-first. Added alongside existing simple commands (e.g. near `get_app_version` in `lib.rs`, or in a small new `src-tauri/src/app_launch.rs` module if `lib.rs` is judged too cluttered by the time of implementation) rather than piggy-backing on `api.rs`, since `api.rs` is reserved for chat/STT/provider concerns per the OpenWiki "where to change things" table. This command has the same "no existing Rust-side SQL query pattern" gap as the insert above and should reuse the same `sqlx::SqlitePool` (or a pooled connection to it) established for the startup insert, rather than opening a third connection.
...
- [No prior art in this codebase for running SQL from Rust against `pluely.db`; all existing access is frontend-only via `@tauri-apps/plugin-sql`] → This change introduces that pattern for the first time (a directly-managed `sqlx::SqlitePool` opened by app code, separate from the plugin's internal pool). Mitigated by confirming the resolved db file path matches what the plugin uses and adding this as an explicit implementation task (see Decisions above) rather than assuming it "already works like other setup code."
