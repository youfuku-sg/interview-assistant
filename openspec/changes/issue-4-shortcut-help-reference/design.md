...
- **Keep the Settings reference synchronized with the editor's in-memory state.** `ShortcutManager` currently loads bindings into component-local React state and writes localStorage when a row changes. A sibling component that independently calls `getShortcutsConfig()` only on mount would therefore remain stale after an edit, toggle, or reset on the same `/shortcuts` visit. The Settings integration must provide a small state-notification path (for example, an optional `onBindingsChange` callback from `ShortcutManager`, with the page passing the resulting bindings to `ShortcutReference`) so the read-only list updates immediately after every successful local state change. The Help page may load fresh data on mount because navigating to it mounts a new page. This synchronization does not change persistence or backend behavior.
