...
- **Preview lives in `ThemeProviderContext`, not local component state.** The class toggle on `document.documentElement` already lives in the context's effect; duplicating that logic in `Theme.tsx` would create two places that can flip the `light`/`dark` class and risk them fighting each other. Instead, add an internal `previewedTheme: Theme | null` state value to the provider and derive the *effective displayed* theme as `previewedTheme ?? theme` inside the existing `applyTheme` effect. This effect's dependency array is currently `[theme]` — it must become `[theme, previewedTheme]`, otherwise the effect won't re-run when only `previewedTheme` changes and hovering an option will silently do nothing. The `if (theme === "system") { mediaQuery.addEventListener(...) }` guard (and its cleanup counterpart) must use the same `previewedTheme ?? theme` value so the OS-preference listener is (de)registered correctly while "system" is being previewed.
- **Expose a `previewTheme(theme | null)` action, not the internal state or a separate "commit" flag.** The provider implements this action with its internal `setPreviewedTheme` state setter. Hover-in calls `previewTheme("light" | "dark" | "system")`; hover-out / menu-close calls `previewTheme(null)`, which falls back to the real `theme`. Clicking an item still calls the existing `setTheme(theme)` (which persists), and `setTheme` also clears the internal preview so the dropdown closing doesn't cause a flicker back through the old theme before the new persisted one re-applies.
