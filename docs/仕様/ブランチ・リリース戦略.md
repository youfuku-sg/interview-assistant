...
- **最新の安定ブランチ（`main`）から分岐**し、対応する `feature/*`（[Issue駆動開発フロー](Issue駆動開発フロー.md) では `dev/<Issue番号>`）を `git merge` で取り込む。`feature/*` から直接分岐しないのは、準備期間中に他の release が先に `main` へ入っている可能性があり、release ブランチは常に最新の `main` を基準にすべきだからである
