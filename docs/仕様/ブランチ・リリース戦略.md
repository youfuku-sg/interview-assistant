...
`develop` は 2026-08-22 から導入した。導入前は使っていなかったため、それ以前の記述・過去のコミット履歴には `develop` を前提としないものが残っている。`build/<name>` ブランチも同日に廃止した（§3.4）。
...
- ビルドが成功したら `feature/<name>` を使って base=`develop` の PR を作成し、ユーザーがビルド済み成果物をダウンロードして動作検証できるようにする。この PR はソースコードの差分レビューを目的としたものではない
- ビルドの失敗、または動作検証での問題発見時は `feature/<name>` を修正して再度ビルドを実行する（成功・検証 OK になるまでのループ）
- 動作検証で問題がなければユーザーがこの PR を `develop` へマージする。このマージが OpenSpec アーカイブ実行の合図になる（[Issue駆動開発フロー](Issue駆動開発フロー.md) 参照）。以降は `feature/<name>` から `release/v<version>` を分岐してリリースフローへ進む
