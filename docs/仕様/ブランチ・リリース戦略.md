...
- `develop`: 開発ブランチ。AI 開発ツール関連のファイル一式（`.claude`/`.codex`/`openspec`/`openwiki`/`docs`/`AGENTS.md`/`CLAUDE.md`）を含む、`feature/*` の分岐元・合流先
...
`develop` は 2026-08-22 から導入した。導入前は使っていなかったため、それ以前の記述・過去のコミット履歴には `develop` を前提としないものが残っている。
...
- ビルドが成功したら `build/<name>` を使って base=`develop` の PR を作成し、ユーザーがビルド済み成果物をダウンロードして動作検証できるようにする。この PR はソースコードの差分レビューを目的としたものではない。`build/*` 自体は他のブランチへマージしない
- ビルドの失敗、または動作検証での問題発見時は `feature/<name>` を修正し、AI が再度 `build/<name>` へ push する（成功・検証 OK になるまでのループ）
- 動作検証で問題がなければユーザーがこの PR を `develop` へマージする。このマージが OpenSpec アーカイブ実行の合図になる（[Issue駆動開発フロー](Issue駆動開発フロー.md) 参照）。マージ後 `build/<name>` は削除する。以降は `feature/<name>` から `release/v<version>` を分岐してリリースフローへ進む
...
- **最新の安定ブランチ（`main`）から分岐**し、対応する `feature/<Issue番号>`（[Issue駆動開発フロー](Issue駆動開発フロー.md) 参照。`develop` にマージ済みの状態のもの）を `git merge` で取り込む。`develop` を丸ごとではなく個別の `feature/*` を取り込むのは `1 release = 1 feature` を保つためであり、`feature/*` から直接分岐しないのは、準備期間中に他の release が先に `main` へ入っている可能性があり、release ブランチは常に最新の `main` を基準にすべきだからである
...
1. `feature/<name>` で作業し、[Issue駆動開発フロー](Issue駆動開発フロー.md) の開発フローに従って dev PR の `develop` へのマージ・OpenSpec アーカイブまで完了させる（この時点で `openspec/specs/` への反映済み）
2. `feature/<name>` から `release/v<version>` を作る（`main` から分岐し `feature/<name>` を merge する。§3.5 参照）
3. `VERSION` と `CHANGELOG.md` を更新する
4. AI 開発ツール関連のファイル（`.claude`, `.codex`, `openspec`, `AGENTS.md`, `CLAUDE.md`, `docs`, `openwiki`）を release ブランチ上で取り除く（`git rm -r --ignore-unmatch`）。`.github/workflows/` は GitHub Actions の動作に必要なため対象外、`CHANGELOG.md` は対象外
5. GitHub Actions を手動実行、または `v<version>` タグ push でビルドする
6. draft release に成果物を添付する
7. インストールして最終確認する（動作検証自体は開発フローの「ユーザー動作検証」で完了済みのため、ここではバージョン更新・ファイル除去後のビルドが健全であることの最終チェックとして行う）
8. 問題なければ安定ブランチ（`main`）へマージする
9. 必要に応じて `v<version>` タグを作る
10. `openwiki-update.yml` による `develop` 上の OpenWiki の自動更新を確認する（`main` からは 4 で除去済みのため対象外。次回の Issue 起点で `develop` 上の最新の現状仕様を参照できる状態を維持する）
11. `release/*` ブランチを削除する
